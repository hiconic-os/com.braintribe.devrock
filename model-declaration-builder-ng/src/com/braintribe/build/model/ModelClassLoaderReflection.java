...
public class ModelClassLoaderReflection implements ModelReflection {
	...
	private Class<? extends Annotation> gmSystemInterfaceClass = null;
	...
	public static ModelReflection scan(ClassLoader classLoader) {
		...
		try {
			...
			tools.gmSystemInterfaceClass = Class.forName(GmSystemInterface.class.getName(), false, classLoader).asSubclass(Annotation.class);
			...
			throw Exceptions.unchecked(e1, "Error while loading special classes from secondary classloader: classpath of model hierarchy is broken",
	...
	public Entity load(String className) {
		try {
			...
			Entity cn = new Entity(className, isGenericEntity(suspect), isGmEnum(suspect), isForward(suspect));
			return cn;
		} catch (Exception e) {
	...
	private boolean isGenericEntity(Class<?> clazz) {
		return genericEntityClass.isAssignableFrom(clazz) && clazz.getAnnotation(gmSystemInterfaceClass) == null;
	}

	private boolean isGmEnum(Class<?> clazz) {
		return clazz.isEnum() && clazz.getAnnotation(gmSystemInterfaceClass) == null;
	}
}